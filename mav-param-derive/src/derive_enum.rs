...
use syn::{parse::ParseBuffer, parse_macro_input, Attribute, Data, DeriveInput, Expr, Fields, Ident, Lit, LitInt, Variant};
use quote::quote;
...
pub fn enum_derive(input: TokenStream) -> TokenStream {
    ...
    let variant_infos = variants.iter().map(|variant| {
        ...
        (variant.ident.clone(), discriminant)
    }).collect::<Vec<_>>();

    let value_variant = get_repr_value(&input.attrs);

    let discriminant_arms = variant_infos.iter().map(|(name, disc)| {
        quote! { Self::#name(_) => #value_variant(#disc), }
    });

    let set_discriminant_arms = variant_infos.iter().map(|(name, disc)| {
        quote! { #value_variant(#disc) => *self = Self::#name(Default::default()), }
    });

    let discriminant_list = variant_infos.iter().map(|(_, disc)| {
        quote! { #value_variant(#disc), }
    });

    let active_variant_ref_arms = variant_infos.iter().map(|(name, _)| {
        quote! { Self::#name(inner) => inner.node_ref(), }
    });

    let active_variant_mut_arms = variant_infos.iter().map(|(name, _)| {
